...
function getRandomCards(items: CardModel[], count: number): CardModel[] {
  const selection: CardModel[] = []

  for (let i = 0; i < count; i++) {
    const randomIndex = (Math.random() * items.length) | 0 // randomIndex va nous donner un chiffre à virgule donc on met | 0 pour mettre en entier
